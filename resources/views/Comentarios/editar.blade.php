...
<div class="page-header d-xl-flex d-block">
    <div class="page-leftheader">
        ...
        <ul class="breadcrumb">
            <li class="mb-1 fs-16"><a href="{{ route('registro4.index') }}">Registro4</a></li>
...
<div class="row">
	<div class="col-xl-12 col-md-12 col-lg-12">
		<div class="card">
			<form method="POST" action="{{route('registro4.update', $comentario->id)}}" enctype="multipart/form-data">
				@method('PATCH')
				@csrf
			<div class="card-body">

				<h4 class="mb-5 font-weight-semibold">Edita el registro {{$comentario->id}} </h4>
				<div class="row">
					<div class="col-md-3">
						<div class="form-group">
							<label for="name" class="form-label">Nombre</label>
							<input id="name" class="form-control @error('name') is-invalid @enderror" type="text" maxlength="22"
                                   name="name" autofocus required value="{{ $comentario->name }}">
                            @error('name')
				...
				<!--<div class="col-md-3">
						<div class="form-group">
							...
							<input id="location" class="form-control @error('location') is-invalid @enderror" type="text" maxlength="100"
                                   name="location" required value="{{ $comentario->location }}">
                            @error('location')
                                <span class="invalid-feedback" role="alert">
                                    <strong> {{ $message }}</strong>
                                </span>
					...
					<div class="col-md-2">
						<div class="form-group">
							...
							<input id="views_counter" class="form-control @error('views_counter') is-invalid @enderror" type="number" maxlength="30"
                                   name="views_counter" required min="1" value="{{ $comentario->views_counter }}">
                            @error('views_counter')
                                <span class="invalid-feedback" role="alert">
                                    <strong> {{ $message }}</strong>
                                </span>
					...
					<div class="col-md-4">
						<div class="form-group">
							...
							<input id="total_cost" class="form-control @error('total_cost') is-invalid @enderror" type="number" step="0.01"
                                   placeholder="50.00" name="total_cost" required min="0" value="{{ $comentario->total_cost }}">
                            @error('total_cost')
                                <span class="invalid-feedback" role="alert">
                                    <strong> {{ $message }}</strong>
                                </span>
